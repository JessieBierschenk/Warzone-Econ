...
For the difference-difference analysis, we will do the proportionate change in GDP from the previous year.
```{r}
subset_join_agg_1  = subset(subset_join_agg, reg == "UA_43")
subset_join_agg_1 = subset_join_agg_1 %>% 
  arrange(year) 
subset_join_agg_1 = subset_join_agg_1 %>% 
  mutate(Diff_year = year - lag(year, default=first(year)),  # Difference in time (just in case there are gaps)
